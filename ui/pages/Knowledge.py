...
from agent_with_knowledge import get_knowledge_agent
...
st.set_page_config(
    page_title="Knowledge Agent: The Research Agent",
...
async def header():
    st.markdown("<h1 class='heading'>Knowledge Agent</h1>", unsafe_allow_html=True)
    st.markdown(
...
async def body() -> None:
    ...
    if (
        ...
        st.session_state[agent_name]["agent"] = knowledge_agent
        st.session_state["selected_model"] = model_id
    else:
        knowledge_agent = st.session_state[agent_name]["agent"]

    ####################################################################
    # Load Agent Session from the database
    ####################################################################
    try:
        st.session_state[agent_name]["session_id"] = knowledge_agent.load_session()
    except Exception:
        st.warning("Could not create Knowledge Agent session, is the database running?")
        return

    ####################################################################
    # Load agent runs (i.e. chat history) from memory is messages is empty
    ####################################################################
    if knowledge_agent.memory:
        agent_runs = knowledge_agent.memory.runs
        if len(agent_runs) > 0:
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":thinking_face: Thinking..."):
                ...
                try:
                    ...
                    run_response = await knowledge_agent.arun(user_message, stream=True)
                    ...
                    if knowledge_agent.run_response is not None:
                        await add_message(agent_name, "assistant", response, knowledge_agent.run_response.tools)
                    else:
    ...
    # Session selector
    ####################################################################
    await session_selector(agent_name, knowledge_agent, get_knowledge_agent, user_id, model_id)

    ####################################################################
    # About section
    ####################################################################
    await utilities_widget(agent_name, knowledge_agent)
