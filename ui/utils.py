...
async def example_inputs(agent_name: str) -> None:
    ...
    with st.sidebar:
        ...
        if agent_name == "sage":
            ...
            if st.button("Tell me about retention analysis"):
                await add_message(
                    agent_name,
                    "user",
                    "Tell me about retention analysis",
                )
        elif agent_name == "query_decomposition":
            if st.button("What is the average retention rate for the last 30 days?"):
                await add_message(
                    agent_name,
                    "user",
                    "What is the average retention rate for the last 30 days?",
                )
